...
extern "C" {
    ...
    MHandle         ASLFA_CreateEngine();
    MVoid           ASLFA_DestroyEngine(MHandle hHandle);
    
    MRESULT         ASLFA_Initialize(MHandle hHandle,
                                     const MChar* szTrackDataPath,
                                     MUInt32 nProcessFaceCount,
                                     MVoid* JNIEnv, MVoid** jcontext);
    MRESULT         ASLFA_Uninitialize(MHandle hHandle);
    
    MRESULT         ASLFA_Process(MHandle hHandle,
    ...
    MRESULT			ASLFA_GetFaceStatus(MHandle hHandle, LPASLFA_FACE_STATUS pFaceStatusOut);
    
    const ASVL_VERSION* ASLFA_GetVersion();
